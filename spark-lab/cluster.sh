...
function startServices {
  ...
  docker exec -u hadoop -d nodemaster jupyter notebook --ip=0.0.0.0 --port=8888 --no-browser --notebook-dir='/opt/spark-apps' --NotebookApp.token='' --NotebookApp.password='' --allow-root
...
if [[ $1 = "deploy" ]]; then
  ...
  docker run -d -p 8081:8081 -p 8080:8080 -p 8088:8088 -p 7077:7077 -p 9870:9870 -p 4040:4040 -p 8888:8888 -p 18080:18080 --network sparknet --name nodemaster -h nodemaster -v `pwd`/spark-apps:/opt/spark-apps -it hhmida/sparkbase
  docker run -dP --network sparknet --name node2 -it -h node2 -v `pwd`/spark-apps:/opt/spark-apps hhmida/sparkbase
  docker run -dP --network sparknet --name node3 -it -h node3 -v `pwd`/spark-apps:/opt/spark-apps hhmida/sparkbase
